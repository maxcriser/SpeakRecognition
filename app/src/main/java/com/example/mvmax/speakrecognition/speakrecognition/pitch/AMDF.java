...
public class AMDF implements PitchDetector {
    ...
    AMDF(final float sampleRate, final int bufferSize) {
        this(sampleRate, bufferSize, DEFAULT_MIN_FREQUENCY, DEFAULT_MAX_FREQUENCY);
    }

    private AMDF(final float sampleRate, final int bufferSize, final double minFrequency, final double maxFrequency) {
    ...
    public PitchDetectionResult getPitch(final float[] audioBuffer) {
        ...
        final int maxShift = audioBuffer.length;
        ...
        for (int i = 0; i < maxShift; i++) {
            ...
            final int frameLength = frames1.length;
        ...
        final int cutoff = (int) Math.round((sensitivity * (maxval - minval)) + minval);
        int j = (int) minPeriod;
        ...
        final double search_length = minPeriod / 2;
        minval = amd[j];
        int minpos = j;
        int i = j;
